		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (pCamCalData->sensorID == DALIOV08F10ULTRA_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x0205, 1, &vendor_id);
	} else if (pCamCalData->sensorID == TURNEROV08F10FRONT_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x0201, 1, &vendor_id);
	} else if (pCamCalData->sensorID == KLEESC821CSULTRA_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x829D, 1, &vendor_id);
	} else if (pCamCalData->sensorID == DASHSC821CSULTRA_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x829D, 1, &vendor_id);
	} else {
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	/* get AWB enable bit */
	if(pCamCalData->sensorID == 0x0800){
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			0x38B2, 1, (unsigned char *)&AWBConfig);
	} else if(pCamCalData->sensorID == DALIOV08F10ULTRA_SENSOR_ID){ /*dali ov08f*/
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			0x0954, 1, (unsigned char *)&AWBConfig);
	} else if(pCamCalData->sensorID == TURNEROV08F10FRONT_SENSOR_ID){ /*turner ov08f*/
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			0x0943, 1, (unsigned char *)&AWBConfig);
	} else if (pCamCalData->sensorID == KLEESC821CSULTRA_SENSOR_ID) {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			0x8AB7, 1, (unsigned char *)&AWBConfig);
	} else if (pCamCalData->sensorID == DASHSC821CSULTRA_SENSOR_ID) {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			0x8AB7, 1, (unsigned char *)&AWBConfig);
	} else {
	...
	/* get AF enable bit */
	if (pCamCalData->sensorID != KLEESC821CSULTRA_SENSOR_ID && pCamCalData->sensorID != DASHSC821CSULTRA_SENSOR_ID) {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (table_size > 0) {
        ...
        if (RODINIMX882WIDE_SENSOR_ID == pCamCalData->sensorID)
		pCamCalData->SingleLsc.TableRotation = 1;
        if (GOYAOV13BULTRA_SENSOR_ID == pCamCalData->sensorID)
		pCamCalData->SingleLsc.TableRotation = 1;
        if (CHAGALLOV13BULTRA_SENSOR_ID == pCamCalData->sensorID)
		pCamCalData->SingleLsc.TableRotation = 1;
#endif
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#ifdef __XIAOMI_CAMERA__
	if (0x0800 == pCamCalData->sensorID || DALIOVX8000WIDE_SENSOR_ID == pCamCalData->sensorID || KLIMTOVX9100WIDE_SENSOR_ID == pCamCalData->sensorID|| CHAGALLOVX8000WIDE_SENSOR_ID == pCamCalData->sensorID) {
        read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		if(pCamCalData->PDAF.Data[1] == 0x37){
			CAM_CAL_LOG_ERR("pd step1 byte2 error, modify to 0x35");
			pCamCalData->PDAF.Data[1] = 0x35;
		}

		if(pCamCalData->PDAF.Data[977] == 0x37){
			CAM_CAL_LOG_ERR("pd step2 byte2 error, modify to 0x35");
			pCamCalData->PDAF.Data[977] = 0x35;
		}
		...
		if(KLIMTS5KJN5TELE_SENSOR_ID == pCamCalData->sensorID || KLIMTS5KJN5TELE_V2_SENSOR_ID == pCamCalData->sensorID || CHAGALLS5KJN5TELE_SENSOR_ID== pCamCalData->sensorID){
			read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
									(start_addr + pCamCalData->PDAF.Size_of_PDAF + 2), 1004*2,
									(u8 *)&pCamCalData->PDAF.Data[1500]);
			pCamCalData->PDAF.Size_of_PDAF = 3508;

			//O12U tele step 2
			CAM_CAL_LOG_ERR("klimt tele pdaf step2, before 0x%x/0x%x, 0x%x/0x%x, 0x%x/0x%x",
				pCamCalData->PDAF.Data[1316],
				pCamCalData->PDAF.Data[1317],
				pCamCalData->PDAF.Data[2320],
				pCamCalData->PDAF.Data[2321],
				pCamCalData->PDAF.Data[3324],
				pCamCalData->PDAF.Data[3325]);

			pCamCalData->PDAF.Data[1316] = 0x33;
			pCamCalData->PDAF.Data[1317] = 0x32;

			pCamCalData->PDAF.Data[2320] = 0x33;
			pCamCalData->PDAF.Data[2321] = 0x31;

			pCamCalData->PDAF.Data[3324] = 0x33;
			pCamCalData->PDAF.Data[3325] = 0x33;

			CAM_CAL_LOG_ERR("klimt tele pdaf step2, after 0x%x/0x%x, 0x%x/0x%x, 0x%x/0x%x",
				pCamCalData->PDAF.Data[1316],
				pCamCalData->PDAF.Data[1317],
				pCamCalData->PDAF.Data[2320],
				pCamCalData->PDAF.Data[2321],
	...
	if(0x906 == pCamCalData->sensorID || RODINIMX882WIDE_SENSOR_ID == pCamCalData->sensorID || KLEEIMX882WIDE_SENSOR_ID == pCamCalData->sensorID){
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,

...
static void mtk_cam_ctrl_dynamic_raws_change_flow(struct mtk_cam_job *job)
{
	...
	dev_info(dev, "[%s] start to try uninit engine ois_en:%d, uninit raw:0x%x\n",
			__func__, ois_comp, job->raw_change_uninit_engine);
...
static int mtk_cam_watchdog_monitor_vsync(struct mtk_cam_watchdog *wd)
{
	...
	for (i = 0; i < ARRAY_SIZE(ctx->hw_raw); i++) {
		if (ctx->hw_raw[i]) {
			...
			if (raw_dev->enable_irq_failed) {
				mtk_cam_event_error(ctrl, MSG_ENABLE_IRQ_ERROR);
				raw_dev->enable_irq_failed = false;
			}
		}
	}

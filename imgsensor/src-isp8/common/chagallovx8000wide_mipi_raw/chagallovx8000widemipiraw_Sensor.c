 ...
 * Filename:
 * ---------
 *	 chagallovx8000widemipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "chagallovx8000widemipiraw_Sensor.h"
...
static int chagallovx8000wide_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int chagallovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt);
static int chagallovx8000wide_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void chagallovx8000wide_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static int chagallovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void mi_stream(void *arg,bool enable);
static void mi_read_CGRatio(void *arg);
static int chagallovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int chagallovx8000wide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
#define SEAMLESS_SWITCH_GROUP_THIRD_PART_MODE                 6
#define REG_ADDR_3830_VALUE                                   7
#define STAGGER2_MODE_DEFAULT_HIGH_EXPOSURE                   1536
#define STAGGER2_MODE_DEFAULT_LOW_EXPOSURE                    768



/* STRUCT */
static struct mtk_sensor_saturation_info chagallovx8000_saturation_info_14bit = {
	.gain_ratio = 3400,
	.OB_pedestal = 64,
...
static struct mtk_sensor_saturation_info chagallovx8000_saturation_info_fake14bit = {
	.gain_ratio = 1000,
	.OB_pedestal = 1024,
...
//static u32 chagallovx8000_dcg_ratio_table_14bit[] = {16000};

static struct mtk_sensor_saturation_info chagallovx8000_saturation_info_10bit = {
	.gain_ratio = 1000,
	.OB_pedestal = 64,
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, chagallovx8000wide_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, chagallovx8000wide_seamless_switch},
	{SENSOR_FEATURE_SET_AWB_GAIN, chagallovx8000wide_set_awb_gain},
	{SENSOR_FEATURE_SET_MULTI_DIG_GAIN, chagallovx8000wide_set_multi_dig_gain},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME,chagallovx8000wide_set_multi_shutter_frame_length},
	{SENSOR_FEATURE_SET_ESHUTTER, chagallovx8000wide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, chagallovx8000wide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_DUAL_GAIN,chagallovx8000wide_set_hdr_tri_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER,chagallovx8000wide_set_hdr_tri_shutter},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, chagallovx8000wide_set_max_framerate_by_scenario},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4Crop = {
		...
		{0, 0}, {2048, 1536}, {2816, 2112},{0, 0}, {0, 0},
	},
};
...
/*
 mi_mode_type defu :0 ; full size: 1; bining 2; DXG: 3; CMS: 4;
*/

#if 1 // mode_struct_setting_x
#define mode_struct_setting_0  \
{\
		.frame_desc = frame_desc_prev,\
		.num_entries = ARRAY_SIZE(frame_desc_prev),\
		.mode_setting_table = chagallovx8000wide_setting_0,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_0),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_0,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_0),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_1  \
{\
		.frame_desc = frame_desc_cus8,\
		.num_entries = ARRAY_SIZE(frame_desc_cus8),\
		.mode_setting_table = chagallovx8000wide_setting_1,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_1),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_1,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_1),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75150000,\
...
#define mode_struct_setting_2  \
{\
		.frame_desc = frame_desc_cus9,\
		.num_entries = ARRAY_SIZE(frame_desc_cus9),\
		.mode_setting_table = chagallovx8000wide_setting_2,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_2),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_2,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_2),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_3  \
{\
		.frame_desc = frame_desc_cus4,\
		.num_entries = ARRAY_SIZE(frame_desc_cus4),\
		.mode_setting_table = chagallovx8000wide_setting_3,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_3),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_3,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_3),\
		...
		.saturation_info = &chagallovx8000_saturation_info_fake14bit,\
		.awb_enabled = true,\
}

#define mode_struct_setting_4  \
{\
		.frame_desc = frame_desc_cus5,\
		.num_entries = ARRAY_SIZE(frame_desc_cus5),\
		.mode_setting_table = chagallovx8000wide_setting_4,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_4),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_4,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_4),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_5  \
{\
		.frame_desc = frame_desc_cus7,\
		.num_entries = ARRAY_SIZE(frame_desc_cus7),\
		.mode_setting_table = chagallovx8000wide_setting_5,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_5),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_5,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_5),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_6  \
{\
		.frame_desc = frame_desc_cus13,\
		.num_entries = ARRAY_SIZE(frame_desc_cus13),\
		.mode_setting_table = chagallovx8000wide_setting_6,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_6),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_6,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_6),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75187500,\
...
#define mode_struct_setting_7  \
{\
		.frame_desc = frame_desc_cus14,\
		.num_entries = ARRAY_SIZE(frame_desc_cus14),\
		.mode_setting_table = chagallovx8000wide_setting_7,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_7),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_7,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_7),\
		...
		.delay_frame = 2,\
		.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW14_B,\
		.saturation_info = &chagallovx8000_saturation_info_14bit,\
}

#define mode_struct_setting_8  \
{\
		.frame_desc = frame_desc_cus15,\
		.num_entries = ARRAY_SIZE(frame_desc_cus15),\
		.mode_setting_table = chagallovx8000wide_setting_8,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_8),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_8,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_8),\
...
#define mode_struct_setting_9 \
{\
		.frame_desc = frame_desc_cus39,\
		.num_entries = ARRAY_SIZE(frame_desc_cus39),\
		.mode_setting_table = chagallovx8000wide_setting_9,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_9),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_9,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_9),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_10  \
{\
		.frame_desc = frame_desc_cus17,\
		.num_entries = ARRAY_SIZE(frame_desc_cus17),\
		.mode_setting_table = chagallovx8000wide_setting_10,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_10),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_10,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_10),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_11  \
{\
		.frame_desc = frame_desc_cus18,\
		.num_entries = ARRAY_SIZE(frame_desc_cus18),\
		.mode_setting_table = chagallovx8000wide_setting_11,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_11),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_11,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_11),\
		.hdr_mode = HDR_RAW_DCG_COMPOSE,\
		.raw_cnt = 1,\
		.exp_cnt = 2,\
		.pclk = 75000000,\
...
#define mode_struct_setting_12  \
{\
		.frame_desc = frame_desc_cus19,\
		.num_entries = ARRAY_SIZE(frame_desc_cus19),\
		.mode_setting_table = chagallovx8000wide_setting_12,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_12),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_12,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_12),\
		.hdr_mode = PARAM_UNDEFINED,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_13  \
{\
		.frame_desc = frame_desc_vid,\
		.num_entries = ARRAY_SIZE(frame_desc_vid),\
		.mode_setting_table = chagallovx8000wide_setting_13,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_13),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_13,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_13),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_14  \
{\
		.frame_desc = frame_desc_hs_vid,\
		.num_entries = ARRAY_SIZE(frame_desc_hs_vid),\
		.mode_setting_table = chagallovx8000wide_setting_14,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_14),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_14,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_14),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_16  \
{\
		.frame_desc = frame_desc_cus1,\
		.num_entries = ARRAY_SIZE(frame_desc_cus1),\
		.mode_setting_table = chagallovx8000wide_setting_16,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_16),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_16,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_16),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_17  \
{\
		.frame_desc = frame_desc_cus2,\
		.num_entries = ARRAY_SIZE(frame_desc_cus2),\
		.mode_setting_table = chagallovx8000wide_setting_17,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_17),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_17,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_17),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_18  \
{\
		.frame_desc = frame_desc_cus3,\
		.num_entries = ARRAY_SIZE(frame_desc_cus3),\
		.mode_setting_table = chagallovx8000wide_setting_18,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_18),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_18,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_18),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_19  \
{\
		.frame_desc = frame_desc_cus10,\
		.num_entries = ARRAY_SIZE(frame_desc_cus10),\
		.mode_setting_table = chagallovx8000wide_setting_19,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_19),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_19,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_19),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_19_24fps  \
{\
		.frame_desc = frame_desc_cus10,\
		.num_entries = ARRAY_SIZE(frame_desc_cus10),\
		.mode_setting_table = chagallovx8000wide_setting_19,\
...
#define mode_struct_setting_20  \
{\
		.frame_desc = frame_desc_cus11,\
		.num_entries = ARRAY_SIZE(frame_desc_cus11),\
		.mode_setting_table = chagallovx8000wide_setting_20,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_20),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_20,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_20),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_21  \
{\
		.frame_desc = frame_desc_cus12,\
		.num_entries = ARRAY_SIZE(frame_desc_cus12),\
		.mode_setting_table = chagallovx8000wide_setting_21,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_21),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_21,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_21),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_22  \
{\
		.frame_desc = frame_desc_cus20,\
		.num_entries = ARRAY_SIZE(frame_desc_cus20),\
		.mode_setting_table = chagallovx8000wide_setting_22,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_22),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_22,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_22),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_23  \
{\
		.frame_desc = frame_desc_cus21,\
		.num_entries = ARRAY_SIZE(frame_desc_cus21),\
		.mode_setting_table = chagallovx8000wide_setting_23,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_23),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_23,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_23),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_24  \
{\
		.frame_desc = frame_desc_cus22,\
		.num_entries = ARRAY_SIZE(frame_desc_cus22),\
		.mode_setting_table = chagallovx8000wide_setting_24,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_24),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_THIRD_PART_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_24,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_24),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
#define mode_struct_setting_25  \
{\
		.frame_desc = frame_desc_cus23,\
		.num_entries = ARRAY_SIZE(frame_desc_cus23),\
		.mode_setting_table = chagallovx8000wide_setting_25,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_25),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_THIRD_PART_MODE,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_25,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_25),\
		.hdr_mode = HDR_RAW_DCG_COMPOSE,\
		.raw_cnt = 1,\
		.exp_cnt = 2,\
		.pclk = 75000000,\
...
/*
#define mode_struct_setting_unused_for_mode10_38  \
{\
		.frame_desc = frame_desc_cus6,\
		.num_entries = ARRAY_SIZE(frame_desc_cus6),\
		.mode_setting_table = chagallovx8000wide_setting_unused_for_mode10_38,\
		.mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_unused_for_mode10_38),\
		.seamless_switch_group = PARAM_UNDEFINED,\
		.seamless_switch_mode_setting_table = chagallovx8000wide_setting_unused_for_mode10_38,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(chagallovx8000wide_setting_unused_for_mode10_38),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75000000,\
...
static struct subdrv_mode_struct mode_struct[] = {
	mode_struct_setting_0,		//mode 0 preview
	mode_struct_setting_0,		//mode 1
	mode_struct_setting_13,		//mode 2
	mode_struct_setting_14,		//mode 3
	mode_struct_setting_0,		//mode 4
	mode_struct_setting_16,		//mode 5
	mode_struct_setting_17,		//mode 6
	mode_struct_setting_18,		//mode 7
	mode_struct_setting_3,		//mode 8
	mode_struct_setting_4,		//mode 9
	mode_struct_setting_0,		//mode 10 unused
	mode_struct_setting_5,		//mode 11
	mode_struct_setting_1,		//mode 12
	mode_struct_setting_2,		//mode 13
	mode_struct_setting_19,		//mode 14
	mode_struct_setting_20,		//mode 15
	mode_struct_setting_21,		//mode 16
	mode_struct_setting_6,		//mode 17
	mode_struct_setting_7,		//mode 18
	mode_struct_setting_8,		//mode 19
	mode_struct_setting_0,		//mode 20 unused
	mode_struct_setting_10,		//mode 21
	mode_struct_setting_11,		//mode 22
	mode_struct_setting_12,		//mode 23
	mode_struct_setting_22,		//mode 24
	mode_struct_setting_23,		//mode 25
	mode_struct_setting_24,		//mode 26
	mode_struct_setting_25,		//mode 27
	mode_struct_setting_19_24fps,		//mode 28
	mode_struct_setting_19_24fps,		//mode 29
	mode_struct_setting_0,		//mode 30 unused
	mode_struct_setting_0,		//mode 31 unused
	mode_struct_setting_0,		//mode 32 unused
	mode_struct_setting_6,		//mode 33
	mode_struct_setting_7,		//mode 34
	mode_struct_setting_8,		//mode 35
	mode_struct_setting_3,		//mode 36
	mode_struct_setting_4,		//mode 37
	mode_struct_setting_0,		//mode 38 unused
	mode_struct_setting_5,		//mode 39
	mode_struct_setting_19,		//mode 40
	mode_struct_setting_20,		//mode 41
	mode_struct_setting_21,		//mode 42
	mode_struct_setting_9,		//mode 43
	mode_struct_setting_9,		//mode 44
};

static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CHAGALLOVX8000WIDE_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = chagallovx8000wide_ana_gain_table,
	.ana_gain_table_size = sizeof(chagallovx8000wide_ana_gain_table),
	.tuning_iso_base = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.exposure_step = 4,
	.exposure_margin = 32,
	.saturation_info = &chagallovx8000_saturation_info_10bit,
	...
	.init_setting_table = chagallovx8000wide_init_setting,
	.init_setting_len = ARRAY_SIZE(chagallovx8000wide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = chagallovx8000wide_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = common_open,
...
const struct subdrv_entry chagallovx8000wide_mipi_raw_entry = {
    .name = "chagallovx8000wide_mipi_raw",
    .id = CHAGALLOVX8000WIDE_SENSOR_ID,
    .pw_seq = pw_seq,
...
void mi_stream_check_stream_off(struct subdrv_ctx *ctx)
{
	...
	if ((ctx->is_seamless == FALSE) && (ctx->fast_mode_on == FALSE))
		return;
	...
	for (i = 0; i < timeout; i++) {
		...
		if(framecnt != subdrv_ixc_rd_u8(ctx, ctx->s_ctx.reg_addr_frame_count) && (ctx->fast_mode_on == FALSE))
			return;
	}
...
static void mi_stream(void *arg,bool enable)
{
	...
	if(enable){
		chagallovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &len);
		subdrv_i2c_wr_u8(ctx,0x0304,0x03);
		mdelay(1);
		i2c_table_write(ctx,chagallovx8000wide_stream_on,ARRAY_SIZE(chagallovx8000wide_stream_on));
	}else{
		mi_stream_check_stream_off(ctx);
		i2c_table_write(ctx,chagallovx8000wide_stream_off,ARRAY_SIZE(chagallovx8000wide_stream_off));
	}
}
...
void chagallovx8000wide_update_mode_info(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	if (scenario_id >= ctx->s_ctx.sensor_mode_num)
	{
		DRV_LOGE(ctx, "invalid sid:%u, mode_num:%u\n",
...
void chagallovx8000wide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
//		dol_cnt = ctx->s_ctx.mode[ctx->current_scenario_id].exp_cnt;
	...
	//add check +
	if (ctx->sof_cnt < 4) {
		regVal1 = subdrv_ixc_rd_u8(ctx, 0x0304);
		regVal2 = subdrv_ixc_rd_u8(ctx, 0x0305);
		DRV_LOG_MUST(ctx, "pll1_divp:(0x0304:0x%02x)(0x0305,0x%02x)\n", regVal1, regVal2);
	}
	//add check -
		DRV_LOG(ctx,
...
void chagallovx8000wide_set_multi_shutter_frame_length_dol(struct subdrv_ctx *ctx,
...
if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
	chagallovx8000wide_write_frame_length(ctx, ctx->frame_length);
else if (ctx->s_ctx.reg_addr_auto_extend)
	chagallovx8000wide_write_frame_length(ctx, ctx->min_frame_length);
/* write shutter */
switch (exp_cnt) {
case 1:
...
static int chagallovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	DRV_LOG(ctx, "chagallovx8000wide_set_hdr_tri_shutter");

	chagallovx8000wide_set_multi_shutter_frame_length_dol(ctx, values, exp_cnt, 0);
	return 0;
}
...
static void set_mi_init_setting_seq(void *arg)
{
	...
	subdrv_i2c_wr_u8(ctx, 0x0103, 0x01);// init delay 10ms
	mdelay(10);
	DRV_LOG_MUST(ctx, "EXIT %s\n", __FUNCTION__);
}
...
static int chagallovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	u32 exp_cnt = 0;
	u16 chagallovx8000wide_setting_seamless[700] = {0};
	u32 setting_seamless_len = 0;
	u16 is_staggertolinear = false;
	u32 retLen = 0;
	...
	setting_seamless_len = covert_seamless_setting_table(scenario_id, chagallovx8000wide_setting_seamless);
	...
	chagallovx8000wide_update_mode_info(ctx, scenario_id);

	if (is_staggertolinear) {
		i2c_table_write(ctx,chagallovx8000wide_seamless_switch_stagger2linear_step1,
			ARRAY_SIZE(chagallovx8000wide_seamless_switch_stagger2linear_step1));
	} else {
		i2c_table_write(ctx,chagallovx8000wide_seamless_switch_linear2other_step1,
			ARRAY_SIZE(chagallovx8000wide_seamless_switch_linear2other_step1));
	}

	if (ctx->s_ctx.mode[scenario_id].seamless_switch_group == SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE ||
		ctx->s_ctx.mode[scenario_id].seamless_switch_group == SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE_LCICA ||
		ctx->s_ctx.mode[scenario_id].seamless_switch_group == SEAMLESS_SWITCH_GROUP_HD_CAP_MODE ||
		ctx->s_ctx.mode[scenario_id].seamless_switch_group == SEAMLESS_SWITCH_GROUP_VIDEO_MODE ||
		ctx->s_ctx.mode[scenario_id].seamless_switch_group == SEAMLESS_SWITCH_GROUP_THIRD_PART_MODE) {
		// dont use simplified seamless setting table
		i2c_table_write(ctx,
			ctx->s_ctx.mode[scenario_id].seamless_switch_mode_setting_table,
			ctx->s_ctx.mode[scenario_id].seamless_switch_mode_setting_len);
	 } else {
		i2c_table_write(ctx,chagallovx8000wide_setting_seamless, 2*setting_seamless_len);
	 }
	DRV_LOG(ctx, "seamless switch, group = %d \n", ctx->s_ctx.mode[scenario_id].seamless_switch_group);

	if (ctx->s_ctx.mode[scenario_id].mi_mode_type == 1)
		chagallovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &retLen);
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			chagallovx8000wide_set_multi_shutter_frame_length_dol(ctx, (u64 *)&ae_ctrl->exposure, exp_cnt, 0);
		...
		case HDR_RAW_DCG_COMPOSE:
			chagallovx8000wide_set_shutter_frame_length(ctx, (u8 *)&ae_ctrl->exposure, len);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode == IMGSENSOR_DCG_DIRECT_MODE) {
				chagallovx8000wide_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			}
			else
				set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		default:
			chagallovx8000wide_set_shutter(ctx, (u8 *)&ae_ctrl->exposure.le_exposure,len);
			set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
	...
	if (is_staggertolinear) {
		i2c_table_write(ctx,chagallovx8000wide_seamless_switch_stagger2linear_step2,
			ARRAY_SIZE(chagallovx8000wide_seamless_switch_stagger2linear_step2));
	} else {
		i2c_table_write(ctx,chagallovx8000wide_seamless_switch_linear2other_step2,
			ARRAY_SIZE(chagallovx8000wide_seamless_switch_linear2other_step2));
	}
...
static u16 chagallovx8000wide_dgain2reg(struct subdrv_ctx *ctx, u32 dgain)
{
...
static int chagallovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = chagallovx8000wide_dgain2reg(ctx, gains[i]);
	}
...
static int chagallovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	chagallovx8000wide_set_multi_gain(ctx,values, exp_cnt);
	return 0;
}
...
static int chagallovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	chagallovx8000wide_set_shutter_frame_length(ctx, para, len);
	return 0;
}

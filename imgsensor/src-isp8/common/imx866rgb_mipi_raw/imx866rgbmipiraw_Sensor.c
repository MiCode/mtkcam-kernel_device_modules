 ...
 *============================================================================
 ****************************************************************************/
#include "imx866rgbmipiraw_Sensor.h"
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, imx866rgb_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, imx866rgb_seamless_switch},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = imx866rgb_preview_setting,
		.mode_setting_len = ARRAY_SIZE(imx866rgb_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = imx866rgb_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx866rgb_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = imx866rgb_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx866rgb_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
	...
	},
	{
		...
		.mode_setting_table = imx866rgb_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(imx866rgb_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = imx866rgb_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(imx866rgb_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = imx866rgb_ana_gain_table,
	.ana_gain_table_size = sizeof(imx866rgb_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 24,
	...
	.init_setting_table = imx866rgb_init_setting,
	.init_setting_len = ARRAY_SIZE(imx866rgb_init_setting),
	.mode = mode_struct,
...
#endif
	{HW_ID_RST, {0}, 1},
	{HW_ID_AVDD2, {1800000, 1800000}, 0}, // power AVDD 1.8V to enable 2.8V ldo
	{HW_ID_AVDD, {2800000, 2800000}, 1000},
	{HW_ID_AVDD3, {1800000, 1800000}, 1000}, // power AVDD2 1.8V to enable 2.8V ldo
...
const struct subdrv_entry imx866rgb_mipi_raw_entry = {
	.name = "imx866rgb_mipi_raw",
	.id = IMX866RGB_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int get_sensor_temperature(void *arg)
{
	...
	temperature = subdrv_ixc_rd_u8(ctx, ctx->s_ctx.reg_addr_temp_read);
...
static int imx866rgb_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	subdrv_ixc_wr_u8(ctx, 0x3010, 0x02);
	i2c_table_write(ctx,
...
static int imx866rgb_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	switch (mode) {
	case 5:
		subdrv_ixc_wr_u8(ctx, 0x020E, 0x00); /* dig_gain = 0 */
		break;
	default:
		subdrv_ixc_wr_u8(ctx, 0x0601, mode);
		break;
	}

	if ((ctx->test_pattern) && (mode != ctx->test_pattern)) {
		if (ctx->test_pattern == 5)
			subdrv_ixc_wr_u8(ctx, 0x020E, 0x01);
		else if (mode == 0)
			subdrv_ixc_wr_u8(ctx, 0x0601, 0x00); /* No pattern */
	}

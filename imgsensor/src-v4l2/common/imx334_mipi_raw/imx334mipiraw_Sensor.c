...
#include "imx334mipiraw_Sensor.h"
...
#define imx334_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define IMX334_N_ROUND_UP(n, d) ((((n) + (d) - 1) / (d)) * (d))

/************************Modify Following Strings for Debug********************/
#define PFX "IMX334_camera_sensor"
#define LOG_1 LOG_INF("IMX334,MIPI 4LANE\n")
/************************   Modify end    *************************************/
#define HDR_raw12 0
#undef IMX334_24_FPS
...
static kal_uint32 preVMAX;
static kal_uint32 preLgain;
static kal_uint32 preSgain;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX334_SENSOR_ID,
...
#define IMX334MIPI_MaxGainIndex (241)
kal_uint32 imx334MIPI_sensorGainMapping[IMX334MIPI_MaxGainIndex][2] = {
	{64, 0},
	{66, 1},
	{68, 2},
	{70, 3},
...
kal_uint32 imx334MIPI_sensorGain[IMX334MIPI_MaxGainIndex] = {
...
static kal_uint32 gain2reg(struct subdrv_ctx *ctx, const kal_uint32 gain)
{
	...
	for (iI = 0; iI < (IMX334MIPI_MaxGainIndex-1); iI++) {
		if (gain <= imx334MIPI_sensorGainMapping[iI][0])
			break;
	}

	return imx334MIPI_sensorGainMapping[iI][1];
}
...
static void hdr_write_shutter(struct subdrv_ctx *ctx, kal_uint32 LE, kal_uint32 SE)
{
	kal_uint32 TE;
	kal_uint32 FSC;
	kal_uint32 SHR0;
	...
	kal_uint32 VMAX;
	kal_uint32 offset = 9;
	kal_uint32 BRL = imgsensor_info.custom1.grabwindow_height + 20;
	// kal_uint16 realtime_fps;

	TE = LE + SE;

	LOG_INF("E! TE:%d, LE:%d, SE:%d, BRL:%d\n", TE, LE, SE, BRL);

	//VMAX = read_cmos_sensor(ctx, 0x3030)
	//	| read_cmos_sensor(ctx, 0x3031) << 8
	//	| read_cmos_sensor(ctx, 0x3032) << 16;
	VMAX = preVMAX;
	FSC = VMAX * 2;

	if (FSC < TE) {
		FSC = IMX334_N_ROUND_UP(TE, 4);
	VMAX = FSC / 2;
		LOG_INF("FSC=%d < TE=%d, VMAX=%d\n", FSC, TE, VMAX);
	} else if ((ctx->min_frame_length * 2) > TE) {
		FSC = IMX334_N_ROUND_UP((ctx->min_frame_length * 2), 4);
		VMAX = FSC / 2;
		LOG_INF("(2xMin)=%d > TE=%d, VMAX=%d\n",
				 (ctx->min_frame_length * 2), TE, VMAX);
	}

	/*	sony spec: SHR1 restrictions
	 *	1. need to meet 2n+1
	 *	2. 9 <= SHR1 <= RHS1-2
	 */
	SHR1 = offset;

	/*	sony spec: RHS1 restrictions
	 *	1. need to meet 4n+1
	 *	2. SHR1+2 <= RHS1 <= SHR0-9
	 *	3. RHS1 < BRL*2
	 */
	RHS1 = IMX334_N_ROUND_UP((SHR1 + SE), 4) + 1;
	/* DOL mode restrictions */
	if (RHS1 + (2 * BRL) >= FSC) {
		FSC = IMX334_N_ROUND_UP((RHS1 + (2 * BRL) + 50), 4);
		VMAX = FSC / 2;
		LOG_INF("Extend FSC to %d, VMAX:%d, RHS1:%d, 2*BRL:%d\n",
				 FSC, VMAX, RHS1, (2*BRL));
	}

	/*	sony spec: SHR0 restrictions
	 *	1. need to meet 2n
	 *	2. RHS1+9 <= SHR0 <= FSC-2
	 */
	SHR0 = IMX334_N_ROUND_UP((FSC - LE - 50), 2);
	if ((SHR0 - RHS1) < offset) {
		LOG_INF("LE:%d < SE:%d, FSC:%d, SHR0:%d, RHS1:%d\n",
				(FSC-SHR0), (RHS1-SHR1), FSC, SHR0, RHS1);
		FSC = IMX334_N_ROUND_UP(FSC + (offset - (SHR0 - RHS1)), 4);
		VMAX = FSC / 2;
		LOG_INF("Extend FSC to %d, VMAX=%d, SHR0:%d, RHS1:%d\n",
				 FSC, VMAX, SHR0, RHS1);
	}

	ctx->frame_length = FSC;
	...
	FSC = ctx->frame_length;
	VMAX = FSC / 2;

	LOG_INF("[Cal done] FSC=%d, VMAX=%d, SHR0=%d, SHR1=%d, RHS1=%d\n",
		FSC, VMAX, SHR0, SHR1, RHS1);
	...
	if (preSHR0 != SHR0) {
		write_cmos_sensor(ctx, 0x3058, SHR0 & 0xFF);
		write_cmos_sensor(ctx, 0x3059, SHR0 >> 8 & 0xFF);
		write_cmos_sensor(ctx, 0x305A, SHR0 >> 16 & 0xF);
	} else
		LOG_INF("SKIP SHR0! preSHR0:%d = SHR0:%d", preSHR0, SHR0);

	/* SE */
	//write_cmos_sensor(ctx, 0x305C, SHR1 & 0xFF);
	//write_cmos_sensor(ctx, 0x305D, SHR1 >> 8 & 0xFF);
	//write_cmos_sensor(ctx, 0x305E, SHR1 >> 16 & 0xF);
	if (preRHS1 != RHS1) {
		write_cmos_sensor(ctx, 0x3068, RHS1 & 0xFF);
		write_cmos_sensor(ctx, 0x3069, RHS1 >> 8 & 0xFF);
		write_cmos_sensor(ctx, 0x306A, RHS1 >> 16 & 0xF);
	} else
		LOG_INF("SKIP RHS1! preRHS1:%d = RHS1:%d", preRHS1, RHS1);

	if (preRHS1 < RHS1) {
		write_cmos_sensor(ctx, 0x3030, VMAX & 0xFF);
		write_cmos_sensor(ctx, 0x3031, VMAX >> 8 & 0xFF);
		write_cmos_sensor(ctx, 0x3032, VMAX >> 16 & 0xF);
		LOG_INF("preRHS1:%d <= RHS1:%d", preRHS1, RHS1);
	} else
		LOG_INF("SKIP VMAX! preRHS1:%d > RHS1:%d", preRHS1, RHS1);
	...
	LOG_INF("X! FINAL:le:%d, se:%d, FSC:%d, framelength:%d\n",
			FSC-SHR0, RHS1-SHR1, ctx->frame_length, FSC);
}


static void hdr_write_gain(struct subdrv_ctx *ctx, kal_uint32 lgain, kal_uint32 sgain)
{
	kal_uint32 reg_lgain;
	kal_uint32 reg_sgain;

	LOG_INF("E! lgain:0x%x, sgain:0x%x\n", lgain, sgain);
	...
	if (preLgain != reg_lgain) {
		write_cmos_sensor(ctx, 0x30E8, reg_lgain & 0xFF);
		write_cmos_sensor(ctx, 0x30E9, reg_lgain >> 8 & 0x07);
	} else
		LOG_INF("preLgain:%d = reg_lgain:%d\n", preLgain, reg_lgain);

	if (preSgain != reg_sgain) {
		write_cmos_sensor(ctx, 0x30EA, reg_sgain & 0xFF);
		write_cmos_sensor(ctx, 0x30EB, reg_sgain >> 8 & 0x07);
	} else
		LOG_INF("preSgain:%d = reg_sgain:%d\n", preSgain, reg_sgain);

	preLgain = reg_lgain;
	preSgain = reg_sgain;

	LOG_INF("X! lgain:0x%x, sg:0x%x\n", reg_lgain, reg_sgain);
}

//static void dump_sensor_data(struct subdrv_ctx *ctx)
//{
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3000),
//		read_cmos_sensor(ctx, 0x3001),
//		read_cmos_sensor(ctx, 0x3002),
//		read_cmos_sensor(ctx, 0x3003),
//		read_cmos_sensor(ctx, 0x300C),
//		read_cmos_sensor(ctx, 0x300D),
//		read_cmos_sensor(ctx, 0x3018),
//		read_cmos_sensor(ctx, 0x302C));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x302D),
//		read_cmos_sensor(ctx, 0x302E),
//		read_cmos_sensor(ctx, 0x302F),
//		read_cmos_sensor(ctx, 0x3030),
//		read_cmos_sensor(ctx, 0x3031),
//		read_cmos_sensor(ctx, 0x3032),
//		read_cmos_sensor(ctx, 0x3034),
//		read_cmos_sensor(ctx, 0x3035));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3048),
//		read_cmos_sensor(ctx, 0x3049),
//		read_cmos_sensor(ctx, 0x304A),
//		read_cmos_sensor(ctx, 0x304B),
//		read_cmos_sensor(ctx, 0x304C),
//		read_cmos_sensor(ctx, 0x304E),
//		read_cmos_sensor(ctx, 0x304F),
//		read_cmos_sensor(ctx, 0x3050));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3058),
//		read_cmos_sensor(ctx, 0x3059),
//		read_cmos_sensor(ctx, 0x305A),
//		read_cmos_sensor(ctx, 0x305C),
//		read_cmos_sensor(ctx, 0x305D),
//		read_cmos_sensor(ctx, 0x305E),
//		read_cmos_sensor(ctx, 0x3060),
//		read_cmos_sensor(ctx, 0x3061));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3062),
//		read_cmos_sensor(ctx, 0x3064),
//		read_cmos_sensor(ctx, 0x3065),
//		read_cmos_sensor(ctx, 0x3066),
//		read_cmos_sensor(ctx, 0x3068),
//		read_cmos_sensor(ctx, 0x3069),
//		read_cmos_sensor(ctx, 0x306A),
//		read_cmos_sensor(ctx, 0x306C));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x306D),
//		read_cmos_sensor(ctx, 0x306E),
//		read_cmos_sensor(ctx, 0x3074),
//		read_cmos_sensor(ctx, 0x3075),
//		read_cmos_sensor(ctx, 0x3076),
//		read_cmos_sensor(ctx, 0x3077),
//		read_cmos_sensor(ctx, 0x3078),
//		read_cmos_sensor(ctx, 0x3079));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x307A),
//		read_cmos_sensor(ctx, 0x307B),
//		read_cmos_sensor(ctx, 0x3080),
//		read_cmos_sensor(ctx, 0x3081),
//		read_cmos_sensor(ctx, 0x3082),
//		read_cmos_sensor(ctx, 0x3083),
//		read_cmos_sensor(ctx, 0x3088),
//		read_cmos_sensor(ctx, 0x308E));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x308F),
//		read_cmos_sensor(ctx, 0x3090),
//		read_cmos_sensor(ctx, 0x3091),
//		read_cmos_sensor(ctx, 0x3094),
//		read_cmos_sensor(ctx, 0x3095),
//		read_cmos_sensor(ctx, 0x3096),
//		read_cmos_sensor(ctx, 0x309B),
//		read_cmos_sensor(ctx, 0x309C));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x309D),
//		read_cmos_sensor(ctx, 0x309E),
//		read_cmos_sensor(ctx, 0x30A4),
//		read_cmos_sensor(ctx, 0x30A5),
//		read_cmos_sensor(ctx, 0x30B6),
//		read_cmos_sensor(ctx, 0x30B7),
//		read_cmos_sensor(ctx, 0x30C6),
//		read_cmos_sensor(ctx, 0x30C7));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x30CE),
//		read_cmos_sensor(ctx, 0x30CF),
//		read_cmos_sensor(ctx, 0x30D8),
//		read_cmos_sensor(ctx, 0x30D9),
//		read_cmos_sensor(ctx, 0x30E8),
//		read_cmos_sensor(ctx, 0x30E9),
//		read_cmos_sensor(ctx, 0x30EA),
//		read_cmos_sensor(ctx, 0x30EB));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x30EC),
//		read_cmos_sensor(ctx, 0x30ED),
//		read_cmos_sensor(ctx, 0x30EE),
//		read_cmos_sensor(ctx, 0x30EF),
//		read_cmos_sensor(ctx, 0x3116),
//		read_cmos_sensor(ctx, 0x3117),
//		read_cmos_sensor(ctx, 0x314C),
//		read_cmos_sensor(ctx, 0x314D));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x315A),
//		read_cmos_sensor(ctx, 0x3168),
//		read_cmos_sensor(ctx, 0x316A),
//		read_cmos_sensor(ctx, 0x3199),
//		read_cmos_sensor(ctx, 0x319D),
//		read_cmos_sensor(ctx, 0x319E),
//		read_cmos_sensor(ctx, 0x319F),
//		read_cmos_sensor(ctx, 0x31A0));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x31A1),
//		read_cmos_sensor(ctx, 0x31A4),
//		read_cmos_sensor(ctx, 0x31A5),
//		read_cmos_sensor(ctx, 0x31A6),
//		read_cmos_sensor(ctx, 0x31A8),
//		read_cmos_sensor(ctx, 0x31AC),
//		read_cmos_sensor(ctx, 0x31AD),
//		read_cmos_sensor(ctx, 0x31AE));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x31D4),
//		read_cmos_sensor(ctx, 0x31D5),
//		read_cmos_sensor(ctx, 0x31D7),
//		read_cmos_sensor(ctx, 0x31DD),
//		read_cmos_sensor(ctx, 0x31E4),
//		read_cmos_sensor(ctx, 0x31E8),
//		read_cmos_sensor(ctx, 0x31F3),
//		read_cmos_sensor(ctx, 0x3200));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3288),
//		read_cmos_sensor(ctx, 0x328A),
//		read_cmos_sensor(ctx, 0x3300),
//		read_cmos_sensor(ctx, 0x3302),
//		read_cmos_sensor(ctx, 0x3303),
//		read_cmos_sensor(ctx, 0x3308),
//		read_cmos_sensor(ctx, 0x3309),
//		read_cmos_sensor(ctx, 0x3414));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3416),
//		read_cmos_sensor(ctx, 0x341C),
//		read_cmos_sensor(ctx, 0x341D),
//		read_cmos_sensor(ctx, 0x35AC),
//		read_cmos_sensor(ctx, 0x3648),
//		read_cmos_sensor(ctx, 0x364A),
//		read_cmos_sensor(ctx, 0x364C),
//		read_cmos_sensor(ctx, 0x3678));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x367C),
//		read_cmos_sensor(ctx, 0x367E),
//		read_cmos_sensor(ctx, 0x3708),
//		read_cmos_sensor(ctx, 0x3714),
//		read_cmos_sensor(ctx, 0x3715),
//		read_cmos_sensor(ctx, 0x3716),
//		read_cmos_sensor(ctx, 0x3717),
//		read_cmos_sensor(ctx, 0x371C));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x371D),
//		read_cmos_sensor(ctx, 0x372C),
//		read_cmos_sensor(ctx, 0x372D),
//		read_cmos_sensor(ctx, 0x372E),
//		read_cmos_sensor(ctx, 0x372F),
//		read_cmos_sensor(ctx, 0x3730),
//		read_cmos_sensor(ctx, 0x3731),
//		read_cmos_sensor(ctx, 0x3732));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3733),
//		read_cmos_sensor(ctx, 0x3734),
//		read_cmos_sensor(ctx, 0x3735),
//		read_cmos_sensor(ctx, 0x375D),
//		read_cmos_sensor(ctx, 0x375E),
//		read_cmos_sensor(ctx, 0x375F),
//		read_cmos_sensor(ctx, 0x3760),
//		read_cmos_sensor(ctx, 0x3768));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3769),
//		read_cmos_sensor(ctx, 0x376A),
//		read_cmos_sensor(ctx, 0x376B),
//		read_cmos_sensor(ctx, 0x376C),
//		read_cmos_sensor(ctx, 0x376D),
//		read_cmos_sensor(ctx, 0x376E),
//		read_cmos_sensor(ctx, 0x3776),
//		read_cmos_sensor(ctx, 0x3777));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3778),
//		read_cmos_sensor(ctx, 0x3779),
//		read_cmos_sensor(ctx, 0x377A),
//		read_cmos_sensor(ctx, 0x377B),
//		read_cmos_sensor(ctx, 0x377C),
//		read_cmos_sensor(ctx, 0x377D),
//		read_cmos_sensor(ctx, 0x377E),
//		read_cmos_sensor(ctx, 0x377F));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3780),
//		read_cmos_sensor(ctx, 0x3781),
//		read_cmos_sensor(ctx, 0x3782),
//		read_cmos_sensor(ctx, 0x3783),
//		read_cmos_sensor(ctx, 0x3784),
//		read_cmos_sensor(ctx, 0x3788),
//		read_cmos_sensor(ctx, 0x378A),
//		read_cmos_sensor(ctx, 0x378B));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x378C),
//		read_cmos_sensor(ctx, 0x378D),
//		read_cmos_sensor(ctx, 0x378E),
//		read_cmos_sensor(ctx, 0x378F),
//		read_cmos_sensor(ctx, 0x3790),
//		read_cmos_sensor(ctx, 0x3792),
//		read_cmos_sensor(ctx, 0x3794),
//		read_cmos_sensor(ctx, 0x3796));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x37B0),
//		read_cmos_sensor(ctx, 0x3A01),
//		read_cmos_sensor(ctx, 0x3A04),
//		read_cmos_sensor(ctx, 0x3A05),
//		read_cmos_sensor(ctx, 0x3A18),
//		read_cmos_sensor(ctx, 0x3A19),
//		read_cmos_sensor(ctx, 0x3A1A),
//		read_cmos_sensor(ctx, 0x3A1B));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3A1C),
//		read_cmos_sensor(ctx, 0x3A1D),
//		read_cmos_sensor(ctx, 0x3A1E),
//		read_cmos_sensor(ctx, 0x3A1F),
//		read_cmos_sensor(ctx, 0x3A20),
//		read_cmos_sensor(ctx, 0x3A21),
//		read_cmos_sensor(ctx, 0x3A22),
//		read_cmos_sensor(ctx, 0x3A23));
//	LOG_INF("0x%x  0x%x  0x%x  0x%x 0x%x  0x%x  0x%x\n",
//		read_cmos_sensor(ctx, 0x3A24),
//		read_cmos_sensor(ctx, 0x3A25),
//		read_cmos_sensor(ctx, 0x3A26),
//		read_cmos_sensor(ctx, 0x3A27),
//		read_cmos_sensor(ctx, 0x3A28),
//		read_cmos_sensor(ctx, 0x3A29),
//		read_cmos_sensor(ctx, 0x3E04));
//}
			...
			kal_uint32 LE, kal_uint32 ME, kal_uint32 SE)
{
	...
	kal_uint32 SHR2;
	kal_uint32 RHS2;
	//kal_uint32 realtime_fps;

	LOG_INF("E! le:%d[0x%x], me:%d[0x%x], se:%d[0x%x]\n",
			LE, LE, ME, ME, SE, SE);

	TE = LE + ME + SE + 109 + imgsensor_info.margin;
	FSC = ctx->frame_length * 4;
	if (FSC < TE) {
		ctx->frame_length = TE/4 + 4;
		FSC = ctx->frame_length * 4;
	} else
		ctx->frame_length = ctx->min_frame_length;

	if (imgsensor_info.max_frame_length < FSC) {
		FSC = imgsensor_info.max_frame_length;
		ctx->frame_length = TE/4;
	}
	...
	/*	sony spec: RHS1 restrictions
	 *	1. need to meet 6n+1
	 *	2. SHR1+3 <= RHS1 <= SHR2-13
	 *	3. RHS1 < BRL*3
	 */

	RHS1 = IMX334_N_ROUND_UP((SHR1 + ME), 6) + 1;

	/* sony spec: SHR2 restrictions
	 * 1. need to meet 3n+2
	 * 2. RHS+13 <= SHR2 <= RHS2-3
	 */

	SHR2 = IMX334_N_ROUND_UP((RHS1 + SHR1), 3) + 2;

	/* sony spec: RHS2 restrictions
	 * 1. need to meet 6n+2
	 * 2. SHR2+3 <= RHS2 <= SHR0-13
	 * 3. RHS2 < (RHS1+(BRL*3))
	 */

	RHS2 = IMX334_N_ROUND_UP((SHR2 + SE), 6) + 2;
	if (SE <= 3)
		RHS2 = RHS2 + 3;

	/* sony spec: SHR0 restrictions
	 *	1. need to meet 3n
	 *	2. RHS2+13 <= SHR0 <= FSC-3
	 */

	SHR0 = IMX334_N_ROUND_UP((FSC - LE), 3);
	if (LE <= 3)
		SHR0 = IMX334_N_ROUND_UP((SHR0 - 3), 3);

	LOG_INF("SHR0=%d, SHR1=%d, RHS1=%d, SHR2=%d, RHS2=%d\n",
		SHR0, SHR1, RHS1, SHR2, RHS2);
/*
 *	if (ctx->autoflicker_en) {
 ...
 */
	...
	write_cmos_sensor(ctx, 0x3062, SHR2 >> 16 & 0xF);
	write_cmos_sensor(ctx, 0x306C, RHS2 & 0xFF);
	write_cmos_sensor(ctx, 0x306D, RHS2 >> 8 & 0xFF);
	write_cmos_sensor(ctx, 0x306E, RHS2 >> 16 & 0xF);

	write_cmos_sensor(ctx, 0x3001, 0x00);

	LOG_INF("X! FINAL:le:%d[0x%x], me:%d[0x%x], se:%d[0x%x]\n",
			FSC-SHR0, FSC-SHR0, RHS1-SHR1, RHS1-SHR1, RHS2-SHR2, RHS2-SHR2);
	LOG_INF("X! FINAL:framelength:%d, FSC:%d\n",
			ctx->frame_length, FSC);
}


static void hdr_write_tri_gain(struct subdrv_ctx *ctx,
			kal_uint32 lgain, kal_uint32 mgain, kal_uint32 sgain)
{
	kal_uint32 reg_lgain;
	kal_uint32 reg_mgain;
	kal_uint32 reg_sgain;
	...
	LOG_INF("X! lgain:%d[0x%x], mgain:%d[0x%x], sgain:%d[0x%x]\n",
		reg_lgain, reg_lgain, reg_mgain, reg_mgain, reg_sgain, reg_sgain);
...
kal_uint16 addr_data_pair_init_imx334[] = {
	/* INCK setting */
	0x300c, 0x3B,
	0x300D, 0x2A,
	/* vmax */
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor(ctx, 0x304B, 0x01);
...
static void custom1_write_register(struct subdrv_ctx *ctx)
{
	write_cmos_sensor(ctx, 0x3000, 0x01);
	write_cmos_sensor(ctx, 0x3001, 0x00);
	write_cmos_sensor(ctx, 0x3002, 0x01);
	...
	write_cmos_sensor(ctx, 0x302C, 0x3C);
	write_cmos_sensor(ctx, 0x302D, 0x00);
	write_cmos_sensor(ctx, 0x302E, 0x00);
	write_cmos_sensor(ctx, 0x302F, 0x0F);
	write_cmos_sensor(ctx, 0x3030, 0xC4);
	write_cmos_sensor(ctx, 0x3031, 0x09);
	write_cmos_sensor(ctx, 0x3032, 0x00);
	write_cmos_sensor(ctx, 0x3034, 0xEF);
	write_cmos_sensor(ctx, 0x3035, 0x01);
	write_cmos_sensor(ctx, 0x3048, 0x01);
	write_cmos_sensor(ctx, 0x3049, 0x01);
	write_cmos_sensor(ctx, 0x304A, 0x01);
	write_cmos_sensor(ctx, 0x304B, 0x02);
	write_cmos_sensor(ctx, 0x304C, 0x13);
	write_cmos_sensor(ctx, 0x304E, 0x00);
	write_cmos_sensor(ctx, 0x304F, 0x00);
	write_cmos_sensor(ctx, 0x3050, 0x00);
	write_cmos_sensor(ctx, 0x3058, 0xF2);
	...
	write_cmos_sensor(ctx, 0x3065, 0x00);
	write_cmos_sensor(ctx, 0x3066, 0x00);
	write_cmos_sensor(ctx, 0x3068, 0x19);
	write_cmos_sensor(ctx, 0x3069, 0x00);
	write_cmos_sensor(ctx, 0x306A, 0x00);
	write_cmos_sensor(ctx, 0x306C, 0x44);
	write_cmos_sensor(ctx, 0x306D, 0x03);
	write_cmos_sensor(ctx, 0x306E, 0x00);
	write_cmos_sensor(ctx, 0x3074, 0xC0);
	write_cmos_sensor(ctx, 0x3075, 0x00);
	write_cmos_sensor(ctx, 0x3076, 0x74);
	write_cmos_sensor(ctx, 0x3077, 0x08);
	write_cmos_sensor(ctx, 0x3078, 0x02);
	write_cmos_sensor(ctx, 0x3079, 0x00);
	write_cmos_sensor(ctx, 0x307A, 0x00);
	...
	write_cmos_sensor(ctx, 0x3083, 0x00);
	write_cmos_sensor(ctx, 0x3088, 0x02);
	write_cmos_sensor(ctx, 0x308E, 0xC1);
	write_cmos_sensor(ctx, 0x308F, 0x00);
	write_cmos_sensor(ctx, 0x3090, 0x74);
	write_cmos_sensor(ctx, 0x3091, 0x08);
	write_cmos_sensor(ctx, 0x3094, 0x00);
	write_cmos_sensor(ctx, 0x3095, 0x00);
	write_cmos_sensor(ctx, 0x3096, 0x00);
	...
	write_cmos_sensor(ctx, 0x30CE, 0x00);
	write_cmos_sensor(ctx, 0x30CF, 0x00);
	write_cmos_sensor(ctx, 0x30D8, 0x20);
	write_cmos_sensor(ctx, 0x30D9, 0x12);
	write_cmos_sensor(ctx, 0x30E8, 0x00);
	write_cmos_sensor(ctx, 0x30E9, 0x00);
	write_cmos_sensor(ctx, 0x30EA, 0x00);
	write_cmos_sensor(ctx, 0x30EB, 0x00);
	...
	write_cmos_sensor(ctx, 0x31D4, 0x00);
	write_cmos_sensor(ctx, 0x31D5, 0x00);
	write_cmos_sensor(ctx, 0x31D7, 0x01);
	write_cmos_sensor(ctx, 0x31DD, 0x03);
	write_cmos_sensor(ctx, 0x31E4, 0x01);
	write_cmos_sensor(ctx, 0x31E8, 0x00);
	write_cmos_sensor(ctx, 0x31F3, 0x00);
	...
	write_cmos_sensor(ctx, 0x3302, 0x32);
	write_cmos_sensor(ctx, 0x3303, 0x00);
	write_cmos_sensor(ctx, 0x3308, 0x74);
	write_cmos_sensor(ctx, 0x3309, 0x08);
	write_cmos_sensor(ctx, 0x3414, 0x05);
	write_cmos_sensor(ctx, 0x3416, 0x18);
	write_cmos_sensor(ctx, 0x341C, 0xFF);
...
static void seamless_linear_setting(struct subdrv_ctx *ctx)
{
	//write_cmos_sensor(ctx, 0x3000, 0x01);
	//write_cmos_sensor(ctx, 0x3001, 0x00);
	//write_cmos_sensor(ctx, 0x3002, 0x01);
...
static void seamless_2exp_setting(struct subdrv_ctx *ctx)
{
	//write_cmos_sensor(ctx, 0x3000, 0x01);
	//write_cmos_sensor(ctx, 0x3001, 0x00);
	//write_cmos_sensor(ctx, 0x3002, 0x01);
...
kal_uint16 addr_data_pair_preview_imx334[] = {
0x3000, 0x01,
0x3001, 0x00,
0x3002, 0x01,
0x3003, 0x00,
...
{
	LOG_INF("E\n");
	//imx334_table_write_cmos_sensor(ctx, addr_data_pair_preview_imx334,
	//sizeof(addr_data_pair_preview_imx334) / sizeof(kal_uint16));
	sensor_init(ctx);
	/* zvhdr_setting(ctx); */
}				/*    preview_setting  */

kal_uint16 addr_data_pair_custom1_imx334[] = {
0x300C, 0x3B,
0x300D, 0x2A,
0x3048, 0x01,
0x3049, 0x01,
...
#if HDR_raw12
kal_uint16 addr_data_pair_custom2_imx334[] = {
0x300C, 0x3B,
0x300D, 0x2A,
0x3018, 0x04,
0x302C, 0xFC,
...
#endif
static void custom3_write_register(struct subdrv_ctx *ctx)
{
	write_cmos_sensor(ctx, 0x300C, 0x3B);
	write_cmos_sensor(ctx, 0x300D, 0x2A);
	write_cmos_sensor(ctx, 0x3018, 0x04);
	...
	write_cmos_sensor(ctx, 0x302E, 0x80);
	write_cmos_sensor(ctx, 0x302F, 0x07);
	write_cmos_sensor(ctx, 0x3030, 0x65);
	write_cmos_sensor(ctx, 0x3031, 0x04);
	write_cmos_sensor(ctx, 0x3034, 0x4C);
	write_cmos_sensor(ctx, 0x3035, 0x04);
	write_cmos_sensor(ctx, 0x3050, 0x00);
	...
	write_cmos_sensor(ctx, 0x314C, 0x29);
	write_cmos_sensor(ctx, 0x314D, 0x01);
	write_cmos_sensor(ctx, 0x315A, 0x06);
	write_cmos_sensor(ctx, 0x3168, 0xA0);
	write_cmos_sensor(ctx, 0x316A, 0x7E);
	write_cmos_sensor(ctx, 0x319D, 0x00);
	write_cmos_sensor(ctx, 0x319E, 0x02);
	write_cmos_sensor(ctx, 0x31A1, 0x00);
	write_cmos_sensor(ctx, 0x3288, 0x21);
	write_cmos_sensor(ctx, 0x328A, 0x02);
	write_cmos_sensor(ctx, 0x3308, 0x34);
	...
	write_cmos_sensor(ctx, 0x3794, 0x7A);
	write_cmos_sensor(ctx, 0x3796, 0xA1);
	write_cmos_sensor(ctx, 0x3A18, 0x7F);
	write_cmos_sensor(ctx, 0x3A1A, 0x37);
	write_cmos_sensor(ctx, 0x3A1C, 0x37);
	write_cmos_sensor(ctx, 0x3A1E, 0xF7);
	write_cmos_sensor(ctx, 0x3A1F, 0x00);
	write_cmos_sensor(ctx, 0x3A20, 0x3F);
	write_cmos_sensor(ctx, 0x3A22, 0x6F);
	write_cmos_sensor(ctx, 0x3A24, 0x3F);
	write_cmos_sensor(ctx, 0x3A26, 0x5F);
	write_cmos_sensor(ctx, 0x3A28, 0x2F);
	write_cmos_sensor(ctx, 0x3E04, 0x0E);
};
...
/*
 *	imx334_table_write_cmos_sensor(ctx, addr_data_pair_capture_imx334,
...
{
	LOG_INF("%s to call custom1_write_register\n", __func__);
	custom1_write_register(ctx);
	//imx334_table_write_cmos_sensor(ctx, addr_data_pair_custom1_imx334,
	//sizeof(addr_data_pair_custom1_imx334) / sizeof(kal_uint16));
}
...
static void custom2_setting(struct subdrv_ctx *ctx)
{
	...
	//imx334_table_write_cmos_sensor(ctx, addr_data_pair_custom2_imx334,
	//sizeof(addr_data_pair_custom2_imx334) / sizeof(kal_uint16));
}

static void custom3_setting(struct subdrv_ctx *ctx)
{
	custom3_write_register(ctx);
}
...
{
	LOG_INF("Seamless switch to scenario_id %d\n", scenario_id);
	...
	case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
	{
		LOG_INF("seamless switch to linear hdr mode!\n");
		ctx->sensor_mode = IMGSENSOR_MODE_PREVIEW;
		ctx->pclk = imgsensor_info.pre.pclk;
		ctx->line_length = imgsensor_info.pre.linelength;
		ctx->frame_length = imgsensor_info.pre.framelength;
		...
		seamless_linear_setting(ctx);

		if (ae_ctrl) {
			LOG_INF("Shutter:%d, Gain:%d",
					ae_ctrl[SHUTTER_NE], ae_ctrl[GAIN_NE]);
			set_shutter(ctx, ae_ctrl[SHUTTER_NE]);
			set_gain(ctx, ae_ctrl[GAIN_NE]);
		}
	}
		break;
	case SENSOR_SCENARIO_ID_CUSTOM1:
	{
		LOG_INF("seamless switch to 2-DOL mode!\n");
		ctx->sensor_mode = IMGSENSOR_MODE_CUSTOM1;
		ctx->pclk = imgsensor_info.custom1.pclk;
		ctx->line_length = imgsensor_info.custom1.linelength;
		ctx->frame_length = imgsensor_info.custom1.framelength;
		...
		seamless_2exp_setting(ctx);

		if (ae_ctrl) {
			LOG_INF("LE/SE Shutter:%d/%d, Gain:%d/%d",
				ae_ctrl[SHUTTER_NE], ae_ctrl[SHUTTER_SE],
				ae_ctrl[GAIN_NE], ae_ctrl[GAIN_SE]);
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_ESHUTTER:
		set_shutter(ctx, (UINT32)*feature_data);
		break;
	...
	case SENSOR_FEATURE_GET_READOUT_BY_SCENARIO:
		...
		case SENSOR_SCENARIO_ID_NORMAL_CAPTURE:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.cap.linelength
			  * imgsensor_winsize_info[1].h2_tg_size
			  * 1000000000) / imgsensor_info.cap.pclk;
			break;
		case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.normal_video.linelength
			  * imgsensor_winsize_info[2].h2_tg_size
			  * 1000000000) / imgsensor_info.normal_video.pclk;
			break;
		case SENSOR_SCENARIO_ID_HIGHSPEED_VIDEO:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.hs_video.linelength
			  * imgsensor_winsize_info[3].h2_tg_size
			  * 1000000000) / imgsensor_info.hs_video.pclk;
			break;
		case SENSOR_SCENARIO_ID_SLIM_VIDEO:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.slim_video.linelength
			  * imgsensor_winsize_info[4].h2_tg_size
			  * 1000000000) / imgsensor_info.slim_video.pclk;
			break;
		case SENSOR_SCENARIO_ID_CUSTOM1:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.custom1.linelength
			  * imgsensor_winsize_info[5].h2_tg_size * 2
			  * 1000000000) / imgsensor_info.custom1.pclk;
			break;
		case SENSOR_SCENARIO_ID_CUSTOM2:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.custom2.linelength
			  * imgsensor_winsize_info[6].h2_tg_size * 3
			  * 1000000000) / imgsensor_info.custom2.pclk;
			break;
		case SENSOR_SCENARIO_ID_CUSTOM3:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.custom3.linelength
			  * imgsensor_winsize_info[7].h2_tg_size
			  * 1000000000) / imgsensor_info.custom3.pclk;
			break;
		case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
		default:
			(*(feature_data + 1))
			= (MUINT64)((MUINT64)imgsensor_info.pre.linelength
			  * imgsensor_winsize_info[0].h2_tg_size
			  * 1000000000) / imgsensor_info.pre.pclk;
			break;
		}
		break;
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER: // for 2EXP
		...
		hdr_write_shutter(ctx, (UINT32)*feature_data,
			(UINT32)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_DUAL_GAIN:
		...
		hdr_write_gain(ctx, (UINT32)*feature_data,
				(UINT32)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_HDR_TRI_SHUTTER: // for 3EXP
		...
		if (((UINT32)*(feature_data+2) > 1) &&
			((UINT32)*(feature_data+1) > 1) &&
			((UINT32)*(feature_data) > 1))
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx334MIPI_sensorGain);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx334MIPI_sensorGain,
			sizeof(imx334MIPI_sensorGain));
		}
		break;
...
const struct subdrv_entry imx334_mipi_raw_entry = {
	.name = "imx334_mipi_raw",
	.id = IMX334_SENSOR_ID,
	.pw_seq = pw_seq,

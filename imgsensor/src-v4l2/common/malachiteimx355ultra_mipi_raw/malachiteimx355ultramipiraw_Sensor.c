 ...
 * Filename:
 * ---------
 *	 malachiteimx355ultramipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 *============================================================================
 ****************************************************************************/
#include "malachiteimx355ultramipiraw_Sensor.h"

static int get_sensor_temperature(void *arg);
static void set_group_hold(void *arg, u8 en);
static void malachiteimx355ultra_set_dummy(struct subdrv_ctx *ctx);
static int malachiteimx355ultra_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static u16 get_gain2reg(u32 gain);
static int  malachiteimx355ultra_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int ultra_vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt);
static int malachiteimx355ultra_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int malachiteimx355ultra_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int malachiteimx355ultra_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int malachiteimx355ultra_get_csi_param(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id, struct mtk_csi_param *csi_param);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, malachiteimx355ultra_set_test_pattern},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, malachiteimx355ultra_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_ESHUTTER, malachiteimx355ultra_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, malachiteimx355ultra_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, malachiteimx355ultra_set_multi_shutter_frame_length}
};
...
{\
	...
	.mode_setting_table = malachiteimx355ultra_preview_setting,\
	.mode_setting_len = ARRAY_SIZE(malachiteimx355ultra_preview_setting),\
...
{\
	...
	.mode_setting_table = malachiteimx355ultra_normal_video_setting,\
	.mode_setting_len = ARRAY_SIZE(malachiteimx355ultra_normal_video_setting),\
	.seamless_switch_group = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_table = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_len = PARAM_UNDEFINED,\
	.hdr_mode = HDR_NONE,\
...
// mode 5: 3280*2464@24fps bokeh

#define bokeh_mode_struct  \
{\
	.frame_desc = frame_desc_bokeh,\
	.num_entries = ARRAY_SIZE(frame_desc_bokeh),\
	.mode_setting_table = malachiteimx355ultra_custom1_setting,\
	.mode_setting_len = ARRAY_SIZE(malachiteimx355ultra_custom1_setting),\
	.seamless_switch_group = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_table = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_len = PARAM_UNDEFINED,\
	.hdr_mode = HDR_NONE,\
...
#if 0
// mode 6: 1920*1080@60fps
#define custom2_mode_struct  \
{\
	.frame_desc = frame_desc_custom2,\
	.num_entries = ARRAY_SIZE(frame_desc_custom2),\
	.mode_setting_table = malachiteimx355ultra_custom2_setting,\
	.mode_setting_len = ARRAY_SIZE(malachiteimx355ultra_custom2_setting),\
	.seamless_switch_group = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_table = PARAM_UNDEFINED,\
	.seamless_switch_mode_setting_len = PARAM_UNDEFINED,\
	.hdr_mode = HDR_NONE,\
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	bokeh_mode_struct,			//mode 5
	//custom2_mode_struct,		//mode 6
};

static struct subdrv_static_ctx static_ctx = {
	.sensor_id = MALACHITEIMX355ULTRA_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = malachiteimx355ultra_ana_gain_table,
	.ana_gain_table_size = sizeof(malachiteimx355ultra_ana_gain_table),
	.min_gain_iso = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.init_setting_table = malachiteimx355ultra_init_setting,
	.init_setting_len = ARRAY_SIZE(malachiteimx355ultra_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	...
	.get_csi_param = malachiteimx355ultra_get_csi_param,
...
const struct subdrv_entry malachiteimx355ultra_mipi_raw_entry = {
	.name = "malachiteimx355ultra_mipi_raw",
	.id = MALACHITEIMX355ULTRA_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int malachiteimx355ultra_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->frame_length > (ctx->exposure[0] + ctx->s_ctx.exposure_margin))
		malachiteimx355ultra_set_dummy(ctx);
...
static int malachiteimx355ultra_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case 4: // PN9
		...
		break;
	case 5:
		subdrv_i2c_wr_u8(ctx, 0x0602, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0603, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0604, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0605, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0606, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0607, 0x00);
		subdrv_i2c_wr_u8(ctx, 0x0607, 0x00);
		break;
	default:
...
static int malachiteimx355ultra_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	malachiteimx355ultra_set_shutter_frame_length(ctx, para, len);
	return 0;
}
static int malachiteimx355ultra_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
...
static int malachiteimx355ultra_get_csi_param(struct subdrv_ctx *ctx,

 ...
 */
signed int dpe_register_requests_isp8(struct engine_requests *eng, size_t size)
{
	int f, r, d;
	char *_data;
	size_t len;
...
EXPORT_SYMBOL(dpe_register_requests_isp8);

signed int dpe_unregister_requests_isp8(struct engine_requests *eng)
{
	int f, r;
...
EXPORT_SYMBOL(dpe_unregister_requests_isp8);

int dpe_set_engine_ops_isp8(struct engine_requests *eng,
...
EXPORT_SYMBOL(dpe_set_engine_ops_isp8);
...
EXPORT_SYMBOL(dpe_request_running_isp8);
...
EXPORT_SYMBOL(dpe_enque_request_isp8);

/* ConfigWMFERequest / ConfigOCCRequest abstraction
 * TODO: locking should be here NOT camera_owe.c
 */
signed int dpe_request_handler_isp8(struct engine_requests *eng, spinlock_t *lock)
{
	unsigned int f, fn;
	unsigned int r;
...
EXPORT_SYMBOL(dpe_request_handler_isp8);
...
EXPORT_SYMBOL(dpe_update_request_isp8);
...
EXPORT_SYMBOL(dpe_deque_request_isp8);

signed int dpe_request_dump_isp8(struct engine_requests *eng)
{
	unsigned int r = 0;
	unsigned int f = 0;
...
EXPORT_SYMBOL(dpe_request_dump_isp8);
